...
pid_param_t LSpeed_PID;         //左轮PID
pid_param_t RSpeed_PID;         //右轮PID
/*************************************************************************
...
/***************************************************从这里到下面不用改（各个功能的初始化）**********************************************/
	...
	UART_InitConfig(UART0_RX_P14_1,UART0_TX_P14_0, 115200); // 串口P14.0管脚输出,P14.1输入，波特率115200
	...
	IfxCpu_releaseMutex(&mutexCpu0InitIsOk);    // 通知CPU1，CPU0初始化完成

	mutexCpu0TFTIsOk=0;     // CPU1： 0占用/1释放 TFT  切记CPU0,CPU1...不可以同时开启屏幕显示，否则冲突不显示
	ServoInit();            // 舵机初始化
	MotorInit();            // 电机初始化
	EncInit();              // 编码器初始化
