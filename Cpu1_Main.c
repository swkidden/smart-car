...
int core1_main(void) {
  ...
  // CCU6_InitConfig(CCU60, CCU6_Channel0, 10*1000); //zhou 10ms // 100ms进入一次中断
  while (1)  //主循环
  {
    if (Camera_Flag == 2)  //等待DMA传输完成
    {
      Get_Use_Image();                      // 取出赛道及显示所需图像数据 像素大小：60*93
      Get_Bin_Image(3);                     // 转换为01格式数据，0、1原图；2、3边沿提取
      // Eight_neighborhood(3);                //八领域提取
                                            // //Bin_Image_Filter();                   // 滤波，三面被围的数据将被修改为同一数值（不用）
      // UpdownSideGet(Edge_arr, UpdowmSide);  // 提取赛道上边线
      // ImageGetSide(Edge_arr, ImageSide);    // 提取赛道左右边线
      // ImagePortFilter(Bin_Image,Bin_Image);//滤波

       UpdownSideGet(Bin_Image, UpdowmSide);   // 提取赛道上边线
       ImageGetSide(Bin_Image, ImageSide);     // 提取赛道左右边线
      TurnNumSideGet();  // 计算三条边线的折点数
      CameraCar();
